...
class Protoc {
  ...
  start() {
    ...
    exec(
      'protoc ' +
      `--proto_path=${this.projectExportPath} ` +
      `--js_out=import_style=commonjs,binary:${this.projectExportPath} ` +
      protocImports, (error) => {
  ...
  copyPackage(originProtoPath) {
    ...
    const newPackagePath = path.join(this.projectExportPath, filename);
